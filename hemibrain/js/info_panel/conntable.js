    ...
    Overlay)
{
  ...
  function createTemplate(obj){
    var template = "";
    template = "";
    template += '<h4>Associated Lines</h4>';
    template += '<table id="associated_lines" class="table table-inverse table-custom-striped">';
    template += '<thead><tr class=""><th>Line</th> <th>Matching Score</th> <th class="line_add">Show Line</th></tr><tr class=""><th><span class="info-input-span"> Filter by name <br></span><input type="text" id="line-srch" value="" class="info-input"/></th> <th><span class="info-input-span"> Score greater than <br></span><input type="number" id="line-N" value="0" class="info-input selectable"/></th> <th class="line_add"><br></span></th></tr></thead>';
  ...
  */
  ConnTable.prototype.update = function(data){
    ...
    $('#'+this.divId).children('h4').eq(0).html("Associated Lines (NeuronBridge 2.1.0)");
    $('#'+this.divId).children('h4').eq(1).html("Presynaptic Partners"+ btnMoreInfo);
    $('#'+this.divId).children('h4').eq(2).html("Postsynaptic Partners"+ btnMoreInfo);
  ...
  /**
    ...
    */
  ConnTable.prototype.updateLines = function(data){
    var table = $("#associated_lines tbody")[0];
    $("#associated_lines tbody tr").remove();
    ...
    for (x in data['results']) {
      ...
      d = data['results'][x];
      // console.log(d);
      name = d['publishedName'] + ' ' + d['id'];
      N = String(d['normalizedScore']).split(".")[0];

      var row = table.insertRow(-1);
      var c1 = row.insertCell(0);
      var c2 = row.insertCell(1);
      var c3 = row.insertCell(2);
      c3.className = 'add_line'; // remove the . character
      ...
      c2.innerHTML = N;
        ...
        var yOff = 15;
        var xOff = 20;
        let pathToImage = 'https://ffbodata.neuronlp.fruitflybrain.org/bridge/'+d['imageURL'];
        $("#"+btn.id).hover(function (e) {
          // console.log(pathToImage);
          $("body").append("<p id='image-when-hovering-text'><img style='width:512px' src='" + pathToImage + "'/></p>");
        ...
        $("#"+btn.id).click(function (e) {
          openInNewTab('https://neuronbridge.janelia.org/search?q='+d['publishedName']);
      });
    ...
    // refresh list
    this.filterByName('associated_lines',document.getElementById("line-srch").value);
    this.filterByNum('associated_lines',document.getElementById("line-N").value);

    // add callback
    $("#line-srch").on('keyup change',(function(){
      this.filterByName('associated_lines',document.getElementById("line-srch").value);
    }).bind(this));
    $("#line-N").on('keyup change', (function (){
      this.filterByNum('associated_lines',document.getElementById("line-N").value);
    }).bind(this));
